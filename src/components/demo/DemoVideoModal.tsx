...
export const DEMO_VIDEO_SCRIPT = [
  ...
  },
  {
    time: "0:45 - 1:10",
    seconds: 45,
    title: "3. Prizm Driver & 3D Interactive AI Coach",
    badge: "3D Spine Telemetry & Prizm Engine",
    text: "Powered by Three.js biomechanical posture mapping, Nelly—our Google Cloud Agent Builder digital wingman—monitors C1-C7 cervical tilt and L1-L5 lumbar strain in real time. Deeply integrated with Prizm Alert shift fatigue score (/api/v1/fatigue-score), workers receive automated Tier-2 micro-stretch interventions before fatigue turns into acute injury."
  },
  {
    time: "1:10 - 1:30",
    seconds: 70,
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
        className="fixed inset-0 z-[99999] bg-black/90 backdrop-blur-xl flex items-center justify-center p-3 sm:p-6 overflow-x-hidden overflow-y-auto"
      >
        <motion.div
          initial={{ scale: 0.95, y: 20 }}
          animate={{ scale: 1, y: 0 }}
          exit={{ scale: 0.95, y: 20 }}
          className="bg-slate-900 border-2 border-ohs-orange/50 rounded-2xl sm:rounded-3xl max-w-5xl w-[95vw] p-4 sm:p-8 space-y-4 sm:space-y-6 relative shadow-[0_0_100px_rgba(249,168,37,0.3)] my-auto max-h-[90vh] max-h-[90dvh] flex flex-col justify-between overflow-y-auto z-50"
        >
          {/* Header */}
